...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options["solo_eventos"]:
            # Check for categories, centers, and events
            if (
                Categoria.objects.exists()
                and CentroCultural.objects.exists()
                and Evento.objects.exists()
            ):
                self.stdout.write(
                    self.style.SUCCESS(
                        "Datos ya existen en la base de datos. Saltando población..."
                    )
                )
                return
        else:
            # Check only for events if --solo-eventos
            if Evento.objects.exists():
                self.stdout.write(
                    self.style.SUCCESS(
                        "Eventos ya existen en la base de datos. Saltando población..."
                    )
                )
                return
        ...
        try:
            ...
            try:
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS("Usuario 'editor' ya existía.")
                    )
                ...
                for cat_data in categorias_data:
                    try:
                        ...
                        if created:
                            ...
                            self.stdout.write(
                                f"Categoría ya existía: {categoria.nombre}"
                            )
                    except Exception as e:
                        self.stderr.write(
                            self.style.ERROR(
                                f"Error creando categoría {cat_data['nombre']}: {e}"
                            )
                        )
            else:
                # Si es solo eventos, obtener categorías existentes
                categorias = {cat.nombre: cat for cat in Categoria.objects.all()}
                self.stdout.write(f"Usando {len(categorias)} categorías existentes.")

            # Crear centros culturales (solo si no es --solo-eventos)
            if not options["solo_eventos"]:
                self.stdout.write("Creando centros culturales...")
                centros_data = [
                    {
                ...
                for centro_data in centros_data:
                    try:
                        ...
                        if created:
                            ...
                            if options["unsplash_key"] and centro_data.get(
                                "publicado", True
                            ):
                                try:
                                    imagen = self.descargar_imagen_unsplash(
                                        centro_data["imagen_keyword"],
                                        options["unsplash_key"],
                                    )
                                    if imagen:
                                        centro.imagen.save(
                        ...
                        self.stderr.write(
                            self.style.ERROR(
                                f"Error creando centro cultural {centro_data['nombre']}: {e}"
                            )
                        )
            else:
                # Si es solo eventos, obtener centros existentes
                centros = {
                    centro.nombre: centro for centro in CentroCultural.objects.all()
                }
                self.stdout.write(
                    f"Usando {len(centros)} centros culturales existentes."
                )
            ...
            for evento_data in eventos_data:
                try:
                    ...
                    if created:
                        ...
                        if options["unsplash_key"] and evento_data.get(
                            "publicado", True
                        ):
                            try:
                                imagen = self.descargar_imagen_unsplash(
                                    evento_data["imagen_keyword"],
                                    options["unsplash_key"],
                                )
                                if imagen:
                                    evento.imagen.save(
                    ...
                    self.stderr.write(
                        self.style.ERROR(
                            f"Error procesando evento '{evento_data.get('titulo', 'Desconocido')}': {e}"
                        )
                    )
                    continue
            ...
            total_centros = CentroCultural.objects.count()
            total_eventos = Evento.objects.count()
            total_usuarios = User.objects.count()

            self.stdout.write(
                self.style.SUCCESS(
                    f"Resumen: {total_categorias} categorías, {total_centros} centros, "
                    f"{total_eventos} eventos, {total_usuarios} usuarios"
                )
            )

        except Exception as e:
            self.stderr.write(
                self.style.ERROR(f"Error general durante la población: {e}")
            )
